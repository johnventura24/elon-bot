  ...
  extractAndTrackDeadlines(userName, response) {
    ...
    if (matches) {
      matches.forEach(match => {
        ...
        console.log(`📅 Deadline tracked for ${userName}: ${deadline}`);
...
if (slackEvents) {
  ...
  slackEvents.on('message', async (event) => {
    try {
      ...
      const isThreadReply = !!event.thread_ts; // Any message with thread_ts is a thread reply
      const isChannelMessage = event.channel_type === 'channel';
      
      // Process DMs AND thread replies (whether in DMs or channels)
      if (!isDM && !isThreadReply && !isChannelMessage) {
        console.log('⏭️ Skipping: Not a DM, thread reply, or channel message');
        return; 
      }

      console.log(`📨 Processing ${isThreadReply ? 'THREAD REPLY' : (isDM ? 'DM' : 'CHANNEL MESSAGE')} from user:`, event.user);
      console.log('💬 Message:', event.text);
      console.log('🧵 Thread TS:', event.thread_ts || 'None');
...
Deadline: Get back to me with more specifics within 24 hours. Focus on relentless execution. 🚀`;
      ...
      // CRITICAL FIX: Add thread_ts if this is ANY thread reply
      if (event.thread_ts) {
        messagePayload.thread_ts = event.thread_ts;
        console.log(`🧵 REPLYING IN THREAD: ${event.thread_ts}`);
        console.log(`📍 Channel: ${event.channel}, Type: ${event.channel_type}`);
      } else {
        console.log(`💬 SENDING NEW MESSAGE (no thread_ts found)`);
        console.log(`📍 Channel: ${event.channel}, Type: ${event.channel_type}`);
      }
      ...
      if (response.ok) {
        console.log(`🚀 ${result.success ? 'AI-powered' : 'Fallback'} Elon response sent to ${userName}`);
        console.log(`✅ Message sent successfully: ${isThreadReply ? 'IN THREAD' : 'AS NEW MESSAGE'}`);
        console.log(`📊 Response TS: ${response.ts}, Thread TS: ${response.thread_ts || 'None'}`);
      } else {
...
Deadline: Try again in 5 minutes. Persistence beats resistance. 🚀`,
          username: 'Elon Musk',
          icon_emoji: ':warning:'
        };

        // FIXED: Maintain thread context for errors too
        if (event.thread_ts) {
          errorPayload.thread_ts = event.thread_ts;
          console.log(`🧵 Sending error response in thread: ${event.thread_ts}`);
        }
  ...
  console.log('🤖 Slack event handlers initialized with ENHANCED THREADING!');
  console.log('🧵 Threading improvements:');
  console.log('   ✅ Enhanced thread detection logic');
  console.log('   ✅ Better event logging');
  console.log('   ✅ Unified response handling');
